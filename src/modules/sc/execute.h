...
public:
    ...
    switch(fun){
        ...
        cout<<"Register "<<erd<<" was set to first byte of memory"<<reg[rs1]+(simm12_64)<<"(signed)"<<endl;
        break;
        case LH:
        rdc=static_cast<int16_t>(mem[reg[rs1]+(simm12_64)]);
        cout<<"Register "<<erd<<" was set to first 2 bytes of memory"<<reg[rs1]+(simm12_64)<<"(signed)"<<endl;
        break;
        case LW:
        rdc=static_cast<int32_t>(mem[reg[rs1]+(simm12_64)]);
        cout<<"Register "<<erd<<" was set to first 4 bytes of memory"<<reg[rs1]+(simm12_64)<<"(signed)"<<endl;
        break;
        case LBU:
        rdc=static_cast<uint8_t>(mem[reg[rs1]+(simm12_64)]);
        cout<<"Register "<<erd<<" was set to first byte of memory"<<reg[rs1]+(simm12_64)<<" (unsigned)"<<endl;
        break;
        case LHU:
        rdc=static_cast<uint16_t>(mem[reg[rs1]+(simm12_64)]);
        cout<<"Register "<<erd<<" was set to first 2 bytes of memory"<<reg[rs1]+(simm12_64)<<" (unsigned)"<<endl;
        break;
        case SB:
        memc=sregrs2_8;
        cout<<"Memory "<<reg[rs1]+(simm12_64)<<" was set to first byte of register "<<ers2<<" (signed)"<<endl;
        break;
        case SH:
        memc=sregrs2_16;
        cout<<"Memory "<<reg[rs1]+(simm12_64)<<" was set to first 2 bytes of register "<<ers2<<" (signed)"<<endl;
        break;
        case SW:
        memc=sregrs2_32;
        cout<<"Memory "<<reg[rs1]+(simm12_64)<<" was set to first 4 bytes of register "<<ers2<<" (signed)"<<endl;
        break;
        case ADDI:
